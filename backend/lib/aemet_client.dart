...
class AemetGatewayResult {
  const AemetGatewayResult({
...
class AemetGatewayClient {
  ...
  Future<Map<String, dynamic>> municipalityDaily(String code) async =>
      (await municipalityDailyWithMetadata(code)).payload;

  Future<AemetGatewayResult> municipalityDailyWithMetadata(String code) async {
    ...
    if (cached != null && now.difference(cached.retrievedAt) <= cacheTtl) {
      return AemetGatewayResult(
    ...
    try {
      ...
      throw StateError('AEMET no disponible: $error');
    }
  }
