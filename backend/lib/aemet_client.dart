...
class AemetGatewayClient {
  ...
  Future<Map<String, dynamic>> municipalityDaily(String code) async {
    if (!RegExp(r'^\\d{5}$').hasMatch(code)) {
      throw ArgumentError.value(code, 'code', 'Código INE de municipio inválido.');
    }

    final cached = _cache[code];
    if (cached != null && DateTime.now().toUtc().difference(cached.retrievedAt) <= cacheTtl) {
      return cached.payload;
    }

    final envelope = await _getJson(Uri.parse('$baseUri/prediccion/especifica/municipio/diaria/$code'), includeApiKey: true);
    final dataUrl = envelope['datos'];
    if (dataUrl is! String || dataUrl.isEmpty) {
      throw StateError('AEMET no devolvió una URL de datos.');
    }
    final payload = await _getJson(Uri.parse(dataUrl), includeApiKey: false);
    _cache[code] = _CachedResponse(payload: payload, retrievedAt: DateTime.now().toUtc());
    return payload;
  }

  Future<Map<String, dynamic>> _getJson(Uri uri, {required bool includeApiKey}) async {
    Object? lastError;
    for (var attempt = 0; attempt < 3; attempt++) {
      try {
        final response = await _client.get(
          uri,
          headers: includeApiKey ? {'api_key': apiKey} : const {},
...
class _CachedResponse {
  const _CachedResponse({required this.payload, required this.retrievedAt});
