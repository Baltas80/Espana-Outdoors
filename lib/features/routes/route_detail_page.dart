...
class _RouteDetailPageState extends State<RouteDetailPage> {
  ...
  Future<void> _exportTrack() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(saved == null ? 'Exportación cancelada.' : 'GPX guardado.')));
  ...
  Widget build(BuildContext context) {
    ...
    final points = track?.points.map((point) => LatLng(point.latitude, point.longitude)).toList(growable: false) ?? const <LatLng>[];

    return Scaffold(
      appBar: AppBar(title: Text(title), actions: [
        if (track != null) IconButton(tooltip: 'Exportar GPX', onPressed: _exportTrack, icon: const Icon(Icons.file_download_outlined)),
      ]),
      body: ListView(
        ...
        children: [
          SizedBox(
            ...
            child: ClipRRect(
              ...
              child: FlutterMap(
                options: MapOptions(
                  initialCenter: points.isNotEmpty ? points[points.length ~/ 2] : const LatLng(40.4168, -3.7038),
                  initialZoom: points.isNotEmpty ? 12 : 6,
                ),
                children: [
                  TileLayer(
                    urlTemplate: MapProviderConfig.openStreetMapDevelopment.tileUrlTemplate,
                    userAgentPackageName: MapProviderConfig.openStreetMapDevelopment.userAgent,
                  ),
                  RichAttributionWidget(attributions: [TextSourceAttribution(MapProviderConfig.openStreetMapDevelopment.attribution)]),
                  if (points.length > 1) PolylineLayer(polylines: [Polyline(points: points, strokeWidth: 5)]),
                ],
              ),
            ),
          ),
...
class _TrackStats extends StatelessWidget {
  ...
  final ImportedTrack track;
  @override
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(18), child: Wrap(spacing: 20, runSpacing: 16, children: [
...
class _Stat extends StatelessWidget {
  ...
  final String value;
  @override
  Widget build(BuildContext context) => SizedBox(width: 125, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
