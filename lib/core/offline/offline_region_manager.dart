...
import '../map/offline_map_download.dart';
...
class OfflineRegionManager extends ChangeNotifier {
  ...
  Future<void> prepare(String regionId) async {
    ...
    try {
      ...
      if (current != null &&
          current.status == OfflineRegionStatus.downloading) {
        await _store.put(
          current.copyWith(
  ...
  Future<void> _persistProgress(OfflineDownloadProgress progress) async {
    final current = _store.get(progress.region.id);
    if (current == null) return;

    await _store.put(
      current.copyWith(
        status: _statusFor(progress.state),
        progress: progress.fraction ?? current.progress,
        bytesDownloaded: progress.completedBytes,
        bytesTotal: progress.totalBytes ?? current.bytesTotal,
        updatedAt: DateTime.now().toUtc(),
        error: null,
      ),
    );
  ...
  OfflineRegionStatus _statusFor(OfflineDownloadState state) {
    return switch (state) {
      OfflineDownloadState.queued => OfflineRegionStatus.queued,
      OfflineDownloadState.downloading => OfflineRegionStatus.downloading,
      OfflineDownloadState.paused => OfflineRegionStatus.paused,
      OfflineDownloadState.ready => OfflineRegionStatus.ready,
      OfflineDownloadState.failed => OfflineRegionStatus.failed,
      OfflineDownloadState.deleted => OfflineRegionStatus.deleting,
    };
  }
}
