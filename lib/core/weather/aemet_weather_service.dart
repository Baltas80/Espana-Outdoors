...
/// AEMET OpenData adapter. Credentials are runtime-only and expire explicitly.
class AemetWeatherService implements WeatherService {
    ...
    for (final item in days) {
      ...
      parsed.add(
        WeatherDay(
          ...
          precipitationProbabilityPercent: _firstInt(item['probPrecipitacion']),
  ...
  String? _windDirection(dynamic value) {
    if (value is! List || value.isEmpty || value.first is! Map) return null;
    final direction = (value.first as Map)['direccion']?.toString();
  ...
  WeatherCondition _condition(dynamic value) {
    final first = value is List && value.isNotEmpty && value.first is Map
        ? value.first as Map
        : null;
    final normalized = '${first?['descripcion'] ?? ''}'.toLowerCase();
